	...
	_G.Settings = {
		...
		Main = {
			...
			--[World 2]
			["Auto Qua Sea 3"] = false,
			["Đánh Nhà Máy"] = false,
			["Tìm sever đánh nhà mày"] = false,
			["Auto Nhận Nhiệm vụ ba tô lòng mèo"] = false,
	
			["Lấy Tam Bảo Kiếm"] = false,
			["lấy Rengoku"] = false,
			["Lấy Kính Swan"] = false,
			["Lấy áo râu đen"] = false,
			["Farm vật chất"] = false,
	
			["Auto Buy Kiếm Zoro"] = false,
			["Auto Buy Màu Haki"] = false,
	
			--[World 3]
			["Thắp Đuốc Tushita"] = false,
			["Lấy Buddy Swords"] = false,
			["Auto Đánh Boss Hallow"] = false,
			["Auto 7 Màu Haki"] = false,
			["Auto Boss Bí Ẩn"] = false,
			["Lấy Mũ lính ngự lâm "] = false,
			["Auto Buddy Sword"] = false,
			["Auto Farm Xương"] = false,
			["Auto Haki Quang Sáti V2"] = false,
			["Lấy Kiếm Cavander"] = false,
			["Lấy  Yama Sword"] = false,
			["Lấy Tushita Sword"] = false,
			["Lấy Cung Rắn "] = false,
			["Lấy Mini Yoru"] = false,
			["Đánh Katarui"] = false,
			["Đánh Katarui v2"] = false,
			["Auto Quay Xương"] = false,
...
local LogoToggle = Instance.new("ImageLabel")
	...
	LogoToggle.Image = "http://www.roblox.com/asset/?id="..tostring(14395120725)
		...
		function main:Dropdown(text,option,callback)
			...
			DropImage.Image = "rbxassetid://16108379307"
...
local Library = Update:Window("Bacon Hub","2.0",Enum.KeyCode.RightControl);

Main = Library:Tab("Thông tin",6026568198)
Sta = Library:Tab("Nâng điểm",7040410130)
Miscf = Library:Tab("Phần Farm",9606626859)
Sho = Library:Tab("Trái Fruit",7044233235)
...
task.spawn(function()
	while wait() do
		pcall(function()
			...
			elseif _G.Setting_table.SelectWeapon == "" then
				for i ,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
					if v.ToolTip == "Blox " then
						if game.Players.LocalPlayer.Backpack:FindFirstChild(tostring(v.Name)) then
							_G.Setting_table.Weapon = v.Name
						end
					end
...
Main:Toggle("Auto Grab Chest Mirage Island",_G.Setting_table.AutoChestMirage,function(vu)
 ...
 Elite_Hunter_Status = Main:Label("")
	...
	Main:Toggle("Auto Farm ",_G.Setting_table.Farm_,function(vu)
		Auto_Farm_ = vu
		_G.Setting_table.Farm_ = vu
		Update_Setting(getgenv()['MyName'])
	end)
                                    ...
                                    if v.Name == Ms then
                                        ...
                                        repeat task.wait()
                                            if v.Humanoid.Health <= HealthMs then
                                                EquipWeapon(game:GetService("Players").LocalPlayer.Data.Devil.Value)
                                                v.HumanoidRootPart.CanCollide = false
                                                TP(v.HumanoidRootPart.CFrame * CFrame.new(0,30,0))
                                                _G.PosMon = v.HumanoidRootPart.CFrame
                                                v.Humanoid.WalkSpeed = 0
                                                PositionSkillMasteryDevil = v.HumanoidRootPart.Position
                                                UseSkill = true
                                            else           
                                                UseSkill = false 
                                                EquipWeapon(_G.Setting_table.Weapon)
                                            ...
                                            MastertyMagnet = true
                                            Click()
                                        until not Auto_Farm_ or v.Humanoid.Health <= 0 or not v.Parent or game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false
                                    else
                                        UseSkill = false
                                        MastertyMagnet = false
                                    end
        ...
        while wait() do
            if UseSkill then
                pcall(function()
                    CheckLevel()
                    for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if game:GetService("Players").LocalPlayer.Character:FindFirstChild(game:GetService("Players").LocalPlayer.Data.Devil.Value) then
                            MasBF = game:GetService("Players").LocalPlayer.Character[game:GetService("Players").LocalPlayer.Data.Devil.Value].Level.Value
                        elseif game:GetService("Players").LocalPlayer.Backpack:FindFirstChild(game:GetService("Players").LocalPlayer.Data.Devil.Value) then
                            MasBF = game:GetService("Players").LocalPlayer.Backpack[game:GetService("Players").LocalPlayer.Data.Devil.Value].Level.Value
                        end
                        ...
                        elseif game:GetService("Players").LocalPlayer.Character:FindFirstChild(game:GetService("Players").LocalPlayer.Data.Devil.Value) then
                            if SkillZ then 
                                local args = {
                                    [1] = _G.PosMon.Position
                                }
			...
			if tostring(method) == "FireServer" then
				if tostring(args[1]) == "RemoteEvent" then
					if tostring(args[2]) ~= "true" and tostring(args[2]) ~= "false" then
						if UseSkillMasteryDevil then
							args[2] = PositionSkillMasteryDevil
...
Miscf:Button("Teleport To Advanced  Dealer",function()
    for i, ve in pairs(game:GetService("Workspace").NPCs:GetChildren()) do
        if ve.Name == "Advanced  Dealer" then
...
Shop:Button("Buy Surprise",function()
	game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("sBones","Buy",1,1)
end)
...
Main:Label("Bacon Hub hợp tác Banana hub real :))")
